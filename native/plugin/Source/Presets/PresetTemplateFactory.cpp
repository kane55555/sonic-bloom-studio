...
// --- Per-category configurators (kept small to avoid clang-cl debug-info ICE) ---

static void configureDrillBells(HybridPresetV2& p, const juce::String& src, int midi, const juce::String& note)
{
    p.layers.push_back(sampleL(src, midi, note, 0.82f, e(0.001f, 1.2f, 0.15f, 1.8f), lpf(8500.0f)));
    p.layers.push_back(oscL("layer_2","Sine Body","sine",-12,0,0.22f,true, e(0.001f, 0.85f, 0.05f, 1.1f)));
    p.layers.push_back(noiseL("layer_3", true, 0.04f, e(0.001f, 0.06f, 0.0f, 0.04f), "Air Transient"));
    p.layers.push_back(oscL("layer_4","Shimmer","triangle", 12, 7, 0.07f, true, e(0.02f, 1.5f, 0.12f, 2.0f)));
    ...
    p.macros.push_back(mk("macro_2","Space",    0.5f, {{"effects.reverb.mix", 0.0f, 0.7f}, {"effects.delay.mix", 0.0f, 0.4f}}));
...
{
    p.layers.push_back(sampleL(src, midi, note, 0.85f, e(0.001f, 1.0f, 0.85f, 0.8f), lpf(4500.0f, 0.10f, 0.10f)));
...
{
    p.layers.push_back(sampleL(src, midi, note, 0.80f, e(0.05f, 0.6f, 0.85f, 2.5f), lpf(9000.0f)));
    p.layers.push_back(oscL("layer_2","Shimmer","triangle", 12, 0, 0.08f, true, e(0.08f, 1.0f, 0.7f, 2.5f)));
    p.layers.push_back(noiseL("layer_3", true, 0.03f, e(0.4f, 1.5f, 0.4f, 2.0f), "Air"));
    p.layers.push_back(oscL("layer_4","Body Sub","sine", -12, 0, 0.08f, true, e(0.05f, 1.0f, 0.7f, 2.5f)));
...
{
    p.layers.push_back(sampleL(src, midi, note, 0.82f, e(0.003f, 1.2f, 0.55f, 1.6f), lpf(9500.0f)));
    p.layers.push_back(oscL("layer_2","Warm Body","triangle", -12, 0, 0.10f, true, e(0.003f, 1.0f, 0.5f, 1.4f)));
...
{
    p.layers.push_back(sampleL(src, midi, note, 0.80f, e(0.002f, 0.5f, 0.7f, 0.9f), lpf(11000.0f)));
    p.layers.push_back(oscL("layer_2","Square Lead","square", 0, 7, 0.20f, true, e(0.002f, 0.4f, 0.7f, 0.7f)));
    p.layers.push_back(noiseL("layer_3", false, 0.02f, e(0.001f, 0.05f, 0.0f, 0.03f)));
    p.layers.push_back(oscL("layer_4","Sub","sine", -12, 0, 0.10f, true, e(0.002f, 0.4f, 0.7f, 0.7f)));
    ...
    p.macros.push_back(mk("macro_3","Space", 0.5f, {{"effects.reverb.mix", 0.0f, 0.6f}, {"effects.delay.mix", 0.0f, 0.5f}}));
...
{
    p.layers.push_back(sampleL(src, midi, note, 0.82f, e(0.001f, 0.4f, 0.0f, 0.6f), lpf(10000.0f)));
    p.layers.push_back(oscL("layer_2","Body","sine", -12, 0, 0.12f, true, e(0.001f, 0.3f, 0.0f, 0.5f)));
...
{
    p.layers.push_back(sampleL(src, midi, note, 0.78f, e(0.6f, 1.5f, 0.85f, 3.0f), lpf(7000.0f), true, false, true));
    p.layers.push_back(oscL("layer_2","Body","sine", -12, 0, 0.18f, true, e(0.6f, 1.2f, 0.7f, 2.5f)));
    p.layers.push_back(noiseL("layer_3", true, isTexture ? 0.05f : 0.03f, e(0.6f, 1.5f, 0.5f, 3.0f), "Air Wash"));
    p.layers.push_back(oscL("layer_4","Shimmer","triangle", 12, 5, 0.10f, true, e(0.8f, 2.0f, 0.6f, 3.5f)));
...
{
    p.layers.push_back(sampleL(src, midi, note, 0.82f, e(0.005f, 1.0f, 0.4f, 1.2f), lpf(10000.0f)));
                                            ...
                                            const juce::String& rootNote)
{
    ...
    else if (category == "Bass808")      configureBass808      (p, sampleRelPath, rootMidi, rootNote);
    else if (category == "ChoirsVox")    configureChoirsVox    (p, sampleRelPath, rootMidi, rootNote);
    else if (category == "PainPianos")   configurePainPianos   (p, sampleRelPath, rootMidi, rootNote);
    else if (category == "AlienLeads")   configureAlienLeads   (p, sampleRelPath, rootMidi, rootNote);
    else if (category == "Plucks")       configurePlucks       (p, sampleRelPath, rootMidi, rootNote);
    else if (category == "DarkPads")     configureDarkPadsOrTextures(p, sampleRelPath, rootMidi, rootNote, false);
    else if (category == "Textures")     configureDarkPadsOrTextures(p, sampleRelPath, rootMidi, rootNote, true);
    else if (category == "FXRisers")     configureFXRisers     (p, sampleRelPath, rootMidi, rootNote);
